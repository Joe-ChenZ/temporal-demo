...
- The model is Claude (`claude-haiku-4-5`, the cheapest Anthropic model), set on the agent as a **plain name string** `"anthropic/claude-haiku-4-5"`. The plugin only forwards the model *name* (via `_TemporalModelStub`) and resolves it inside the model activity using the plugin's `model_provider` — so `worker.py` passes `model_provider=LitellmProvider()` (a `LitellmModel` *instance* on the agent would be ignored). Without that provider the plugin defaults to an OpenAI provider and builds an `AsyncOpenAI` client at worker startup → demands `OPENAI_API_KEY`. (The SDK also traces to OpenAI by default, but with no `OPENAI_API_KEY` it just logs a harmless `skipping trace export` warning — `agents.set_tracing_disabled(True)` silences it if wanted, but it's optional.) LiteLLM reads `ANTHROPIC_API_KEY` from the environment **when the model activity runs**, so the workflow stays deterministic — `worker.py` calls `load_dotenv()` before litellm is imported; the workflow never reads env.
